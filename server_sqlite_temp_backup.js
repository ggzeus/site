...
// API Pública (Dev Token): Check Foto
// Suporta: /api/check-foto?dev-token=X&user=Y  E TAMBÉM  /api/check-foto/dev-token=X/user=Y
app.get(['/api/check-foto', '/api/check-foto/dev-token=:token/user=:user'], (req, res) => {
    const { width } = req.query;

    // Extract token/user from either Query or Path params
    ...
    if (!token || !username) {
        return res.status(400).json({ status: "error", message: "Parâmetros 'dev-token' e 'user' são obrigatórios." });
    }

    // Valida Token
    db.get("SELECT id FROM users WHERE dev_token = ? AND is_developer = 1", [token], (err, devRow) => {
        if (err) return res.status(500).json({ status: "error", message: "Erro interno no servidor." });
        if (!devRow) {
            return res.status(403).json({ status: "error", message: "Token de desenvolvedor inválido ou inativo." });
        }

        // Busca o usuário alvo
        db.get("SELECT profile_pic FROM users WHERE username = ?", [username], (err, userRow) => {
            if (err) return res.status(500).json({ status: "error", message: "Erro ao buscar usuário." });
            if (!userRow) {
                return res.status(404).json({ status: "error", message: "Usuário não encontrado." });
            }

            // Retorna a URL da foto (ou placeholder se vazia)
            const photoUrl = (userRow.profile_pic && userRow.profile_pic.length > 50)
                ? userRow.profile_pic
                : 'https://i.imgur.com/user_placeholder.png'; // Fallback default
...
app.get('/hwid-reset/:userId', (req, res) => {
    ...
    db.all(`
        SELECT h.*, 
        CASE 
            WHEN h.product_id = -1 THEN 'Todos (Global)'
            ELSE p.name 
        END as product_name 
